...
router.get('/material', expressJwt({ secret: 'adm' }), async (req, res) => {
...
router.get('/material/:id', expressJwt({ secret: 'adm' }), async (req, res) => {
    const { id } = req.params;
    try {
        const vendas = await Venda.findAll({
            where: {
...
router.get('/cliente', expressJwt({ secret: 'adm' }), async (req, res) => {
...
router.get('/transportadora', expressJwt({ secret: 'adm' }), async (req, res) => {
    const { documento } = req.query;
    try {
        const vendas = await Venda.findAll({
            include: [{
