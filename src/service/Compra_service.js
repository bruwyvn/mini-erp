...
router.get('/',expressJwt({ secret: 'compra' }), async (req, res) => {
  try {
...
router.get('/datahora',expressJwt({ secret: 'compra' }), async (req, res) => {
  try {
...
router.get('/descricao',expressJwt({ secret: 'compra' }), async (req, res) => {
  try {
...
router.get('/quantidade', expressJwt({ secret: 'compra' }),async (req, res) => {
  try {
...
router.put('/:id', expressJwt({ secret: 'compra' }),async (req, res) => {
    try {
      const { id } = req.params;
  ...
  router.delete('/:id',expressJwt({ secret: 'compra' }), async (req, res) => {
    try {
      const { id } = req.params;
...
//  criar  compra 
  router.post('/', expressJwt({ secret: 'compra' }),async (req, res) => {
...
router.get('/material/:materialId',expressJwt({ secret: 'compra' }), async (req, res) => {
    try {
      const compras = await Compra.findAll({
  ...
  router.get('/fornecedor/:fornecedorId',expressJwt({ secret: 'compra' }), async (req, res) => {
    try {
      const compras = await Compra.findAll({
