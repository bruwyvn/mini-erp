...
router.get('/',expressJwt({ secret: 'fornecedor' }), async (req, res) => {
  try {
...
router.get('/:id',expressJwt({ secret: 'fornecedor' }), async (req, res) => {
  const { id } = req.params;
  try {
...
router.get('/nome/:nome',expressJwt({ secret: 'fornecedor' }), async (req, res) => {
...
router.get('/documento/:documento', expressJwt({ secret: 'fornecedor' }),async (req, res) => {
...
router.get('/razao-social/:razaoSocial',expressJwt({ secret: 'fornecedor' }), async (req, res) => {
...
router.get('/telefone/:telefone', expressJwt({ secret: 'fornecedor' }),async (req, res) => {
  ...
  router.get('/email/:email', expressJwt({ secret: 'fornecedor' }),async (req, res) => {
...
router.post('/', expressJwt({ secret: 'fornecedor' }),async (req, res) => {
  ...
  router.put('/:id',expressJwt({ secret: 'fornecedor' }), async (req, res) => {
    const { name, documento, razaoSocial, telefone, email, endereco } = req.body;
    const { id } = req.params;
    try {
  ...
  router.delete('/:id',expressJwt({ secret: 'fornecedor' }), async (req, res) => {
    const { id } = req.params;
    try {
      const fornecedor = await Fornecedor.findOne({ where: { id } });
      if (fornecedor) {
