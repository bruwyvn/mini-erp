...
router.get('/',expressJwt({ secret: 'cliente' }),  async (req, res) => {
  try {
...
router.get('/:id', expressJwt({ secret: 'cliente' }), async (req, res) => {
  try {
...
router.get('/endereco/:endereco',expressJwt({ secret: 'cliente' }), async (req, res) => {
  try {
...
router.get('/documento/:documento',expressJwt({ secret: 'cliente' }), async (req, res) => {
  try {
...
router.get('/telefone/:telefone',expressJwt({ secret: 'cliente' }),  async (req, res) => {
  try {
...
router.post('/', expressJwt({ secret: 'cliente' }), async (req, res) => {
...
router.put('/:id', expressJwt({ secret: 'cliente' }), async (req, res) => {
    try {
      const cliente = await Cliente.findByPk(req.params.id);
  ...
  router.delete('/:id',expressJwt({ secret: 'cliente' }),  async (req, res) => {
    try {
      const cliente = await Cliente.findByPk(req.params.id);
